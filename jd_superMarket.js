  ...
  if (smtg_getTeamPkDetailInfoRes && smtg_getTeamPkDetailInfoRes.data.bizCode === 0) {
    ...
    if (joinStatus === 0) {
      if (joinPkTeam === 'true') {
        ...
        await updatePkActivityIdCDN('https://raw.githubusercontent.com/wuzhi-docker1/updateTeam/master/shareCodes/jd_updateTeam.json');
...
//新版东东超市
function updatePkActivityId(url = 'https://raw.githubusercontent.com/wuzhi-docker1/updateTeam/master/shareCodes/jd_updateTeam.json') {
  return new Promise(resolve => {
...
function getTeam() {
  return new Promise(async resolve => {
    $.getTeams = [];
    $.get({url: "https://raw.githubusercontent.com/wuzhi-docker1/updateTeam/master/shareCodes/jd_updateTeam.json",
           headers: {
        "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1 Edg/87.0.4280.88"
    }
}, async (err, resp, data) => {
